 ...
 */
export const checkCalendarReminders = onSchedule(
  {
  ...
  async () => {
    ...
    console.log(
      `Checking reminders for ${dateStr} between ${currentTime} and ${futureTime}`
    );
  ...
  try {
    ...
    if (tokensSnapshot.empty) {
      throw new HttpsError(
        "not-found",
        "لم يتم العثور على أجهزة مسجلة للإشعارات"
      );
    }
    ...
    throw new HttpsError(
      "internal",
      error.message || "فشل في إرسال الإشعار"
    );
  }
});
...
export const scheduledBackup = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      console.log(
        `Scheduled backup completed: ${backupResult.totalDocuments} documents`
      );
    } catch (error) {
 ...
 */
export const triggerCloudBackup = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "يجب تسجيل الدخول أولاً");
  }
  ...
  try {
    ...
    throw new HttpsError(
      "internal",
      error.message || "فشل في إنشاء النسخة الاحتياطية"
    );
  }
});
 ...
 */
export const updateBackupSettings = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "يجب تسجيل الدخول أولاً");
  }
  ...
  try {
    ...
    throw new HttpsError(
      "internal",
      error.message || "فشل في تحديث الإعدادات"
    );
  }
});
...
async function performBackup(type: string, triggeredBy?: string) {
 ...
 */
async function cleanupOldBackups(keep: number) {
  const bucket = admin.storage().bucket();
